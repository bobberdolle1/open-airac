# OpenAIRAC Architecture Specification (v0.3 Routing & Procedures Foundation)
...
11. **`openairac-cli`**: `doctor`, `magnetic`/`magvar`, `magdrift`, `sync` (live OurAirports fetch or `--fixture`), `status`, `validate`, `export xplane` (diagnostic export only; no live-simulator installation).
