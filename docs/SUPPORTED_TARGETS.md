...
| `xplane-gns430` | `gns430-text` | **SUPPORTED** | Transactional subdirectory install into `%XPLANE%/Custom Data/GNS430/navdata` | `%XPLANE%` env, Steam (Windows/Linux), `C:\X-Plane 12` | Full legacy GNS430 schema (Airports, Navaids, Waypoints, ATS, Proc/ per-airport files). Documented in `docs/GNS430_COMPATIBILITY.md` |
| `kln90b` | `kln90b-dat` | **SUPPORTED** | Transactional subdirectory install into KLN90B navdata root | `%KLN90B_NAVDATA%` env, `C:\KLN90B\navdata` | Standard KLN90B `.DAT` tables (APT, NAV, WPT, AWY, FAS). Documented in `docs/KLN90B_COMPATIBILITY.md` |
